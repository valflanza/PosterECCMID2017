...
A2A = read.table("./Enterococcus/all.dist")
...
pairs(pca.all$x[,1:4],col = colores, lower.panel = NULL, pch = 16)
pairs(pca.clara$x[,1:4], col = colores, upper.panel = NULL, pch = 16)
...
tmp %>% gather("Correlation","Value", -X) %>% ggplot(aes(x = X, y = Value, color = Correlation)) + geom_point() + geom_line() + theme_bw() + scale_x_continuous(breaks = c(2:20)) + scale_y_continuous(breaks = seq(0,1,0.05)) + geom_hline(aes(yintercept  =0.8), color = "darkred", linetype = "dotdash") + labs(x ="Number of Clusters", y ="Correlation", title = "Correlation")
